...
class Transform:
    def __init__(self,
        ...
        self.position = initial_position if initial_position is not None else torch.as_tensor([0, 0], dtype=dtype, device=device)
        self.velocity = initial_velocity if initial_velocity is not None else torch.as_tensor([0, 0], dtype=dtype, device=device)
        self.acceleration = initial_acceleration if initial_acceleration is not None else torch.as_tensor([0, 0], dtype=dtype, device=device)
        
        self.angle = initial_angle if initial_angle is not None else torch.as_tensor(0, dtype=dtype, device=device)
        self.angular_velocity = initial_angular_velocity if initial_angular_velocity is not None else torch.as_tensor(0, dtype=dtype, device=device)
    ...
    def GetTransformedVertices(self, mesh : Mesh) -> torch.Tensor:
        return (self.rotation_matrix @ (mesh.vertices - mesh.center)[..., None])[..., 0] + self.position

...
class PolygonSprite(pygame.sprite.Sprite):
    ...
    def UpdateImageRect(self):
        ...
        rotated_image = pygame.transform.rotate(
            image, -torch.rad2deg(self.transform.angle).item()
        )
        ...
        rotated_rect_center = (
            ...
            + (
                ...
                @ (self.mesh.rect_center - self.mesh.center)[..., None]
            )[..., 0]
        )
...
if __name__ == "__main__":
    ...
    test_vertices = torch.as_tensor(
        [[-100, -100], [100, -100], [100, 100], [-100, 100]], dtype=dtype, device=device
    )
    ...
    test_initial_angular_velocity = torch.as_tensor(
        0.5 * torch.pi, dtype=dtype, device=device
    )

    test_transform = Transform(
        initial_position=test_initial_position,
        initial_acceleration=test_initial_acceleration,
        initial_angular_velocity=test_initial_angular_velocity,
    )
    ...
    while running:
        ...
        transformed_test_vertices = (
            test_polygon_sprite.transformed_vertices.cpu().numpy()
        )
        for i in range(4):
            pygame.draw.circle(screen, (255, 255, 255), transformed_test_vertices[i], 5)

        pygame.draw.circle(
            screen, (0, 0, 0), test_polygon_sprite.transform.position.cpu().numpy(), 10
        )
